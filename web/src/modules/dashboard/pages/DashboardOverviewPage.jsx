...
export default function DashboardOverviewPage() {
  ...
  const [activeTabSlot, setActiveTabSlot] = useState('simulator');
  ...
  const handleSimulateNewOrder = (newOrderData) => {
  ...
  const handleResolveChat = (chatId, isAi) => {
    let senderName = '';
    setChats((prev) =>
      prev.map((c) => {
        if (c.id === chatId) {
  ...
  const paymentStats = useMemo(() => {
    const paidAmount = orders
      ...
      .reduce((acc, curr) => acc + curr.amount, 0) + 23530000;

    const pendingAmount = orders
      .filter((o) => o.paymentStatus === 'Pending')
      .reduce((acc, curr) => acc + curr.amount, 0) + 965000;
  ...
  return (
    <div className="flex flex-1 flex-col overflow-y-auto bg-[#f8fafc] -m-4 p-4 pt-3 h-[calc(100vh-58px)] max-h-[calc(100vh-58px)] text-[#1e293b] font-sans antialiased">
      ...
      {/* --- APP HEADER --- */}
      <div className="mb-3 flex shrink-0 flex-col">
