...
export default function ContactPanel({ selected, onUpdate, onDeleteChat }) {
  ...
  const save = async (currentTags, currentNotes) => {
    ...
    setSaving(true)
    try {
      const r = await api.put(`/contacts/${contact.id || contact._id}`, {
        tags: currentTags,
        notes: currentNotes,
      })
      onUpdate?.(r.data)
  ...
  const handleAssign = async (e) => {
    ...
    try {
      const r = await api.post(
        `/chats/${selected.id || selected._id}/takeover`,
        { userId: agentId }
      )
  ...
  if (!selected || !contact) {
    return (
      <div
        className='contact-modern-panel'
        style={{
          alignItems: 'center',
    ...
    <div className='contact-modern-panel'>
      ...
      <div className='contact-content'>
        ...
        <div className='contact-section'>
          <div className='contact-section-title' style={{ marginBottom: 8 }}>
        ...
        <div className='contact-section'>
          <div className='contact-section-header'>
            ...
            <button className='contact-add-btn' onClick={addTag}>
              + Add Label
            </button>
          </div>
          ...
          <div className='contact-label-container'>
            ...
            {tags.map((tag) => (
              <div key={tag} className='contact-label'>
                {tag}
                <span
                  className='contact-label-remove'
