...
export default function ReportsPage() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const [summaryRes, outletRes, productRes, channelRes, dimensionRes] = await Promise.all([
        api.get('/analytics/summary', { params }), api.get('/analytics/outlets', { params }), api.get('/analytics/products', { params }), api.get('/analytics/channels', { params }), api.get('/analytics/dimension', { params: { ...params, dimension: activeDimension } }),
      ])
  ...
  }, [startDate, endDate, selectedOutlet, activeDimension])

  useEffect(() => { fetchReport() }, [fetchReport])

  const rows = useMemo(() => buildReportRows({ dimension: activeDimension, metrics, outlets, outletRows, productRows, channelRows, genericRows }), [activeDimension, metrics, outlets, outletRows, productRows, channelRows, genericRows])
  ...
  const exportReport = () => {
    const data = rows.map((row) => Object.fromEntries([['Dimensi', row.label], ...metrics.map((metric) => [getMetricLabel(metric), metric === 'revenue' || metric === 'averageOrder' ? rupiah(row[metric]) : row[metric]])]))
  ...
  return <div className="min-h-full space-y-5 bg-slate-50 p-5 text-left">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="border-t border-slate-100 px-5 py-4"><label className="mb-2 block text-[11px] font-black uppercase tracking-wide text-slate-400">Dimensi</label><div className="flex flex-wrap items-center gap-2"><MultiSelect label="Tambah Dimensi" items={REPORT_DIMENSIONS} selected={dimensions} onToggle={toggle(setDimensions)} onRemove={(id) => setDimensions((current) => current.filter((item) => item !== id))} />{dimensions.length > 1 && <span className="text-[11px] text-slate-400">Preview MVP memakai dimensi pertama sebagai baris utama.</span>}</div></div>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><h2 className="text-sm font-black text-slate-900">Preview laporan</h2><p className="mt-0.5 text-xs text-slate-400">{getDimensionLabel(activeDimension)} · {startDate} sampai {endDate}</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold text-slate-600">{rows.length} baris</span></div>{error ? <div className="p-10 text-center text-sm font-semibold text-rose-500">{error}</div> : <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-xs"><thead className="bg-slate-50 text-slate-500"><tr><th className="px-5 py-3 text-left font-black">{getDimensionLabel(activeDimension)}</th>{metrics.map((metric) => <th key={metric} className="px-5 py-3 text-right font-black">{getMetricLabel(metric)}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan={metrics.length + 1} className="px-5 py-12 text-center font-semibold text-slate-400">Memuat laporan...</td></tr> : rows.length ? rows.map((row) => <tr key={row.key} className="hover:bg-slate-50"><td className="px-5 py-3 font-bold text-slate-800">{row.label}</td>{metrics.map((metric) => <td key={metric} className="px-5 py-3 text-right font-semibold text-slate-700">{metric === 'revenue' || metric === 'averageOrder' ? rupiah(row[metric]) : Number(row[metric] || 0).toLocaleString('id-ID')}</td>)}</tr>) : <tr><td colSpan={metrics.length + 1} className="px-5 py-12 text-center font-semibold text-slate-400">Belum ada data pada periode dan filter ini.</td></tr>}</tbody>{rows.length > 0 && <tfoot className="border-t-2 border-slate-200 bg-slate-50"><tr><td className="px-5 py-3 font-black text-slate-900">Total</td>{metrics.map((metric) => <td key={metric} className="px-5 py-3 text-right font-black text-slate-900">{metric === 'revenue' || metric === 'averageOrder' ? rupiah(totals[metric]) : Number(totals[metric] || 0).toLocaleString('id-ID')}</td>)}</tr></tfoot>}</table></div>}</section>
  </div>
}
