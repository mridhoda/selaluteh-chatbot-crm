...
export default function PaymentDetailDrawer({
  ...
  useEffect(() => {
    ...
    paymentsApi
      .getEvents(payment._id)
      .then((res) => setEvents(res.data?.events || res.data || []))
      .catch(() => setEvents([]))
      .finally(() => setEventsLoading(false))
  }, [open, payment?._id])

  const isPending = ['pending', 'waiting_payment', 'created'].includes(
    ...
    >
      <div style={{ padding: '0 4px' }}>
        ...
        <Section title='Customer & Outlet'>
          <InfoRow
            label='Customer'
            value={payment.customerName || payment.customer?.name || '—'}
          />
          <InfoRow
            label='Contact'
            value={
          ...
          <InfoRow
            label='Outlet'
            value={payment.outletName || payment.outlet?.name || '—'}
          />
        </Section>
        ...
        {payment.orderId && (
          <Section title='Related Order'>
            ...
            <InfoRow
              label='Order total'
              value={formatIDR(payment.orderTotal)}
            />
