      ...
      <div>
        <span
          style={{
            fontSize: 13,
            color:
              status === 'no' ? 'var(--text-muted)' : 'var(--text-primary)',
          }}
        >
          {label}
        </span>
        {note && (
          <div
            style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 2 }}
          >
            {note}
          </div>
        )}
...
export default function PlatformCapabilitiesChecklist({
  type,
  webhookConfigured,
}) {
  ...
  return (
    <div>
      ...
      <CapRow
        ...
        note={
          isTelegram ? 'Available — commerce backend required' : 'Telegram only'
        }
      />
      <CapRow
        label='Checkout flow'
