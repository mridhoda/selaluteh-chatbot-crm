...
export default function OrderItemsList({
  ...
  const getItemEmojiAndBg = (name = '') => {
    ...
    if (lower.includes('caramel') || lower.includes('sally')) {
      return { emoji: '🧋', bg: 'bg-[var(--brand-50)] border-[var(--brand-100)] text-[var(--brand-500)]' };
    }
    if (lower.includes('kopi') || lower.includes('aren') || lower.includes('coffee')) {
      return { emoji: '🥤', bg: 'bg-[var(--pending-50)] border-[var(--pending-100)] text-[var(--pending-500)]' };
    }
    if (lower.includes('teh') || lower.includes('tea') || lower.includes('lemon')) {
      return { emoji: '🥤', bg: 'bg-[var(--warning-50)] border-[var(--warning-100)] text-[var(--warning-500)]' };
    }
    return { emoji: '🥤', bg: 'bg-[var(--brand-50)] border-[var(--brand-100)] text-[var(--brand-500)]' };
  };
    ...
    <div>
      ...
      <div className="space-y-1.5 text-sm pt-2 border-t border-[var(--border-subtle)]">
        ...
        <div className="flex justify-between font-bold text-[var(--text-primary)] pt-2 border-t border-[var(--border-subtle)]">
          ...
          <span className={`font-extrabold ${paymentStatus === 'Paid' ? 'text-[var(--success-600)]' : 'text-[var(--text-primary)]'}`}>{formatRupiah(total)}</span>
        </div>
      </div>
    </div>
  );
