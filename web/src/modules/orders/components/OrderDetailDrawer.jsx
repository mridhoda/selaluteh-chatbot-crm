    ...
    return (
      <aside className="fixed inset-y-0 right-0 z-[80] w-full md:w-[400px] h-[100dvh] bg-[var(--surface-primary)] border-l border-[var(--border-subtle)] overflow-hidden flex flex-col shrink-0 items-center justify-center text-center p-6 text-[var(--text-muted)] shadow-[-4px_0_15px_rgba(17,24,46,0.03)]">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="text-sm font-semibold text-[var(--text-muted)]">No Order Selected</div>
      ...
      <header className="shrink-0 sticky top-0 z-10 px-5 pt-4 pb-3 border-b border-[var(--brand-100)] bg-[image:var(--orders-sidebar-header-bg)]">
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-bold text-[var(--text-primary)] m-0">Order #{order.orderIdDisplay}</h2>
        ...
        <div className="text-[11px] text-[var(--text-muted)] mb-3">{formattedDateTime}</div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-4">
        ...
        <div>
          ...
          <div className="flex justify-between items-center">
            <div>
              <div className="font-bold text-[var(--text-primary)] text-sm leading-tight">{order.contactId?.name || 'Unknown User'}</div>
              <div className="text-xs text-[var(--text-muted)] mt-1 leading-none">{order.contactId?.phone || '-'}</div>
            </div>
        ...
        <div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-[var(--text-primary)]">{order.paymentMethod || 'Bank Transfer'}</span>
            </div>
            <div className="flex justify-between">
        ...
        {order.notes && (
          <>
            ...
            <div>
              ...
              <div className="text-sm text-[var(--text-secondary)] pl-1">{order.notes}</div>
            </div>
          </>
        )}
