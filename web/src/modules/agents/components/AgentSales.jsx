...
export default function AgentSales({ agent, onUpdate }) {
  ...
  const handleSaveForm = async () => {
    ...
    if (currentFormId) {
      updatedForms = forms.map((f) =>
        (f.id || f._id) === currentFormId
          ? { ...newFormObj, id: f.id, _id: f._id }
          : f
      )
    } else {
  ...
  if (isEditing) {
    return (
      <div
        className='agent-sales-editor card'
      ...
      >
          ...
          {editProducts.map((p, idx) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  <span
                    style={{ fontSize: '0.9em', color: '#888', marginRight: 5 }}
                  >
                    Rp
                  </span>
                  <input
        ...
        <div className='form-group'>
          ...
          <small className='muted' style={{ display: 'block', marginTop: 5 }}>
            Tip: <strong>Item Name</strong> and <strong>Quantity</strong> are
            automatically asked if you added products above.
          </small>
        </div>
  ...
  return (
    <div className='agent-sales-list'>
      ...
      <div
        className='card'
        style={{ padding: 20, marginBottom: 24, border: '1px solid #eee' }}
      >
        <h3>Outlets</h3>
        <div className='row' style={{ gap: 8, marginTop: 10 }}>
          ...
          <button
            ...
            onClick={() => {
              if (newOutlet.trim())
                handleUpdateOutlets([...outlets, newOutlet.trim()])
            }}
          >
            Add
          </button>
            ...
            <div className='form-group'>
              ...
              <textarea
                ...
                onChange={(e) =>
                  onUpdate({
                    ...agent,
                    payment: { ...agent.payment, bankInfo: e.target.value },
                  })
                }
                onBlur={async () => {
                  await api.put(`/agents/${agent.id || agent._id}`, {
            ...
            <div className='form-group' style={{ marginTop: 15 }}>
              ...
              {agent.payment?.qrisUrl ? (
                <div style={{ marginTop: 10 }}>
                  <img
                    src={`${api.defaults.baseURL}${agent.payment.qrisUrl}`}
                    alt='QRIS'
