...
const parsePreviewFileMention = (text = '', databaseFiles = []) => {
  ...
  const altFormatMatch = altText.match(
    /^\{format:(image|sticker|document|file)\}\s*/i
  )
  if (altFormatMatch) {
  ...
  const refFormatMatch = rawRef.match(
    /^\{format:(image|sticker|document|file)\}\s*/i
  )
  if (refFormatMatch) {
  ...
  const filename =
    candidate?.originalName || candidate?.storedName || altText || storedName
  const type =
    explicitFormat === 'image' ||
    explicitFormat === 'sticker' ||
    isImageFilename(filename || storedName)
      ? 'image'
      : 'document'
...
const PreviewAttachment = ({ attachment }) => {
  ...
  return (
    <a
      ...
      className='inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 mt-1 no-underline'
    >
...
export default function AgentDetail() {
  ...
  const getAiFileMarkdown = (file, format = '') => {
    ...
    return `![${formatPrefix}${label}](${link || file.storedName})`
  }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setComplaintFields(
          Array.isArray(a.data.complaintFields) ? a.data.complaintFields : []
        )
        setComplaintNotification(
          a.data.complaintNotification || {
            enabled: false,
            platformId: '',
            destination: '',
          }
  ...
  const sendTest = async () => {
    ...
    if (messages.length === 0) {
      const welcomeMsg = normalizePreviewMessage(
        { from: 'ai', text: welcomeMessage || 'Halo!' },
        database
      )
    ...
    try {
      ...
      if (typeof reply === 'object' && reply.attachment) {
        setMessages((prev) => [
          ...
          normalizePreviewMessage(
            { from: 'ai', text: reply.text, attachment: reply.attachment },
            database
          ),
        ])
      } else {
        setMessages((prev) => [
          ...prev,
          normalizePreviewMessage({ from: 'ai', text: reply }, database),
        ])
      }
    } catch (error) {
      ...
      setMessages((prev) => [
        ...prev,
        { from: 'ai', text: 'Error: Failed to send message' },
      ])
    } finally {
      setTesting(false)
    }
  ...
  const sendQuickReply = async (text) => {
    ...
    try {
      ...
      const r = await api.post(`/agents/${id}/test`, {
        message: text,
    ...
    return (
      <div className='bg-red-50 text-red-700 border border-red-200 rounded-2xl p-6 text-center'>
        ...
        <p className='text-sm mt-1'>
          Kami tidak dapat menemukan konfigurasi agen ini.
        </p>
        <button
          onClick={() => navigate('/app/agents')}
          className='mt-4 bg-red-600 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md hover:bg-red-700'
        >
    ...
    <div className='w-full h-full overflow-y-auto text-slate-800 px-6 py-4 pb-16 box-border'>
      ...
      <header className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4'>
        ...
        <div className='flex items-center gap-3'>
          <span
            className={`text-xs font-medium text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 transition-opacity duration-500 ${saving ? 'opacity-50' : 'opacity-100'}`}
          >
          ...
          <button
            onClick={() => {
              ...
              setFollowUps(
                Array.isArray(agent.followUps) ? agent.followUps : []
              )
              setDatabase(Array.isArray(agent.database) ? agent.database : [])
              setComplaintFields(
                Array.isArray(agent.complaintFields)
                  ? agent.complaintFields
                  : []
              )
              setComplaintNotification(
                agent.complaintNotification || {
                  enabled: false,
                  platformId: '',
                  destination: '',
                }
      ...
      <nav className='flex overflow-x-auto pb-4 mb-4 gap-2 no-scrollbar'>
        {[
          ...
          >
            <i
              className={`fa-solid ${t.icon} ${tab === t.id ? 'text-orange-500' : 'opacity-50'}`}
            ></i>
            {t.label}
          </button>
        ))}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full min-w-0 box-border'>
        ...
        <main
          className={
            tab === 'general'
              ? 'lg:col-span-7 xl:col-span-8 space-y-6 min-w-0 box-border'
              : 'lg:col-span-12 space-y-6 min-w-0 box-border'
          }
        >
          {/* TAB: GENERAL */}
          {tab === 'general' && (
            <div className='flex flex-col gap-6 animate-slide-up w-full max-w-full'>
              ...
              <div className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 transition hover:shadow-2xl hover:shadow-slate-200/80 w-full max-w-full box-border'>
                <div className='flex justify-between items-center mb-4'>
                  <label className='text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2'>
                    <i className='fa-solid fa-fingerprint text-orange-400'></i>{' '}
                    AI Agent Persona
                  </label>
                ...
                <div className='relative group'>
                  ...
                  <button
                    onClick={() => {
                      if (!behavior.trim()) {
                        setBehavior(
                          'Kamu adalah asisten penjualan yang ramah, proaktif, dan siap membantu pelanggan dalam memilih teh dan merchandise premium kami.'
                        )
                      } else {
                        setBehavior(
                          (prev) =>
                            `${prev} Jawablah dengan nada yang ramah, hangat, edukatif tentang varian teh kami, dan bantu pelanggan memproses pesanan dengan sabar.`
                        )
                      }
                    }}
                  ...
                  >
                    <i className='fa-solid fa-wand-magic-sparkles group-hover/btn:animate-pulse'></i>{' '}
                    Improve with AI
                  </button>
                </div>
              </div>
              ...
              <div className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 w-full max-w-full box-border'>
                <label className='text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 block flex items-center gap-2'>
                  <i className='fa-regular fa-message text-orange-400'></i>{' '}
                  Welcome Message
                </label>
                  ...
                  {stickerUrl ? (
                    ...
                    <>
                      ...
                      <p className='text-xs font-medium text-slate-700'>
                        Klik untuk upload gambar selamat datang
                      </p>
              ...
              <div className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 opacity-90 hover:opacity-100 transition w-full max-w-full box-border'>
                <label className='text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 block flex items-center gap-2'>
                  <i className='fa-solid fa-brain text-orange-400'></i> Prompt
                  AI Context
                </label>
            ...
            <div className='flex flex-col gap-6 animate-slide-up'>
              ...
              <div className='bg-white rounded-xl p-1.5 flex gap-1 w-max shadow-sm border border-slate-100'>
                {[
                  ...
                  >
                    <i
                      className={`fa-solid ${sub.icon} ${knowledgeTab === sub.id ? 'text-orange-500' : 'opacity-50'}`}
                    ></i>
                    {sub.label}
                  </button>
                ))}
              ...
              <div className='bg-white rounded-2xl p-8 shadow-xl shadow-slate-200/60 border border-slate-100 min-h-[400px]'>
                <div className='mb-6 border-b border-slate-100 pb-4'>
                  ...
                  <p className='text-sm text-slate-500 mt-1'>
                    Latih bot Anda menggunakan sumber data {knowledgeTab}.
                  </p>
                </div>
                ...
                {knowledgeTab === 'url' && (
                  <div className='bg-slate-50 border border-slate-200 rounded-xl p-5 mb-8'>
                    <h3 className='text-sm font-bold text-slate-700 mb-3 flex items-center gap-2'>
                      <i className='fa-solid fa-plus text-orange-500'></i> Add
                      New URL Source
                    </h3>
                  ...
                  <div className='bg-slate-50 border border-slate-200 rounded-xl p-5 mb-8'>
                    ...
                    >
                      <input
                        ...
                        className='hidden'
                        onChange={(e) =>
                          handleUploadFileKnowledge(e.target.files[0])
                        }
                      />
                      ...
                      {newFileUploading ? (
                        ...
                        <>
                          ...
                          <p className='text-xs text-slate-400 mt-1'>
                            PDF, TXT, DOCX atau CSV (Maks. 10MB)
                          </p>
                        </>
                      )}
                    </div>
                        ...
                        <button
                          onClick={() => {
                            if (newQa.question.trim() && newQa.answer.trim()) {
                              ...
                              alert(
                                'Silakan masukkan pertanyaan dan jawaban terlebih dahulu.'
                              )
                            }
                          }}
                          className='bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-lg font-bold text-sm transition shadow-sm'
                    ...
                    <div className='space-y-3'>
                      {knowledge
                        ...
                        .map((k) => (
                          <div
                            key={k.originalIndex}
                            className='p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center gap-4 animate-slide-up'
                          >
                    ...
                    <div className='space-y-3'>
                      {knowledge
                        ...
                        .map((k) => (
                          <div
                            key={k.originalIndex}
                            className='p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-start gap-4 animate-slide-up'
                          >
                            ...
                            <div className='flex-1'>
                              <KnowledgeTextEditor
                                value={k.value || ''}
                                onChange={(val) =>
                                  updKnowledge(k.originalIndex, { value: val })
                                }
                              />
                            </div>
                            <button
                    ...
                    <div className='space-y-3'>
                      {knowledge
                        ...
                        .map((k) => (
                          <div
                            key={k.originalIndex}
                            className='p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between gap-4 animate-slide-up'
                          >
            ...
            <div className='flex flex-col gap-6 animate-slide-up'>
              <div>
                ...
                <p className='text-sm text-slate-500 mb-6'>
                  Hubungkan agen ke platform messaging yang sudah terdaftar.
                </p>
                  ...
                  <div className='flex items-start justify-between mb-6'>
                    <div className='flex items-center gap-4'>
                      <div className='w-14 h-14 bg-sky-50 rounded-2xl flex items-center justify-center shadow-sm'>
                        <i
                          className={`fa-brands fa-${platforms.find((p) => (p.id || p._id) === platformId)?.type === 'whatsapp' ? 'whatsapp' : 'telegram'} text-3xl text-sky-500`}
                        ></i>
                      </div>
                      <div>
                        <h2 className='text-xl font-bold text-slate-800 capitalize'>
                          {platforms.find((p) => (p.id || p._id) === platformId)
                            ?.type || 'Telegram'}
                        </h2>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mt-4'>
                <div
                  className={`bg-white rounded-2xl p-6 shadow-sm border border-slate-100 transition hover:shadow-lg ${tools.includes('time') ? 'border-orange-300 bg-orange-50/10' : 'opacity-75 hover:opacity-100'}`}
                >
                ...
                <div className='bg-white rounded-2xl p-6 shadow-sm border border-slate-100 opacity-75 hover:opacity-100 transition hover:shadow-lg'>
                  ...
                  <button className='w-full py-2 rounded-lg border border-slate-200 text-slate-400 font-semibold text-sm cursor-not-allowed'>
                    Coming Soon
                  </button>
                </div>
                ...
                <div className='bg-white rounded-2xl p-6 shadow-sm border border-slate-100 opacity-75 hover:opacity-100 transition hover:shadow-lg'>
                  <div className='flex items-center gap-4 mb-4'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-slate-500'>
                        Membaca/menulis data spreadsheet
                      </p>
                    </div>
                  </div>
                  <button className='w-full py-2 rounded-lg border border-slate-200 text-slate-400 font-semibold text-sm cursor-not-allowed'>
            ...
            <div className='flex flex-col gap-6 animate-slide-up'>
              <div className='flex justify-between items-center'>
                <div>
                  ...
                  <p className='text-sm text-slate-500 mt-1'>
                    Sapa kembali pengguna jika mereka berhenti merespons.
                  </p>
                </div>
                <button
                  onClick={addFollowUp}
              ...
              <div className='space-y-6'>
                {followUps.map((f, i) => (
                  <div
                    key={i}
                    className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 relative overflow-hidden group hover:shadow-2xl hover:shadow-slate-200/80 transition-all animate-slide-up'
                  >
          ...
          {tab === 'evaluation' && (
            <div className='bg-white rounded-2xl p-8 border border-slate-100 shadow-xl shadow-slate-200/60 text-center py-16 animate-slide-up'>
              ...
              <p className='text-sm text-slate-500 max-w-md mx-auto mt-1'>
                Tinjau statistik interaksi, tingkat ketepatan, frekuensi
                eskalasi manusia, dan ulasan pelanggan.
              </p>
          ...
          {tab === 'database' && (
            <div className='flex flex-col gap-6 animate-slide-up'>
              <div className='flex justify-between items-center'>
                <div>
                  <h2 className='text-xl font-bold text-slate-800'>
                    Files & Assets Database
                  </h2>
                  <p className='text-sm text-slate-500 mt-1'>
                    Kelola dokumen, gambar, dan file aset untuk bot Anda.
                  </p>
                </div>
              </div>
              ...
              <div className='bg-white rounded-2xl p-8 shadow-xl shadow-slate-200/60 border border-slate-100'>
                ...
                >
                  ...
                  <p className='text-sm text-slate-400 mt-1 max-w-sm mx-auto'>
                    Mendukung Gambar, Dokumen (PDF, DOCX), dan file lainnya.
                    Maks. 25MB.
                  </p>
                </div>
                ...
                {dbUploadStatus.status !== 'idle' && (
                  <div
                    className={`mt-4 p-3 rounded-lg text-sm font-semibold ${
                      dbUploadStatus.status === 'success'
                        ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                        : dbUploadStatus.status === 'error'
                          ? 'bg-red-50 text-red-700 border border-red-200'
                          : 'bg-slate-50 text-slate-700 animate-pulse'
                    }`}
                  >
              ...
              <div>
                <h4 className='text-xs font-bold text-slate-400 uppercase tracking-wider mb-3'>
                  Stored Files ({combinedDatabase.length})
                </h4>
                <div className='grid grid-cols-1 gap-3'>
                  {combinedDatabase.map((f, i) => {
                    const fileKey =
                      f.id || f.storedName || `${f.originalName}-${i}`
                    ...
                    return (
                      <div
                        key={fileKey}
                        className='flex flex-col p-4 bg-white border border-slate-100 rounded-xl hover:shadow-md transition'
                      >
                          ...
                          <div className='mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 animate-slide-up space-y-3'>
                            ...
                            <div>
                                ...
                                <div className='flex gap-1.5'>
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      alert(
                                        'Markdown untuk AI berhasil disalin!'
                                      )
                                    }}
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      alert(
                                        'Markdown sticker berhasil disalin!'
                                      )
                                    }}
                                    className='bg-white border border-slate-200 px-3 py-1 rounded hover:bg-slate-50 text-xs font-semibold text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed'
                                  >
                              ...
                              <input
                                ...
                                value={
                                  getAiFileMarkdown(f) ||
                                  'Upload file ke server dulu agar bisa dipakai AI.'
                                }
                                className='w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono text-slate-600 focus:outline-none'
                                onFocus={(e) => e.target.select()}
                              />
                              <p className='mt-1 text-[11px] text-slate-400'>
                                Auto mengikuti ekstensi file. Pakai Copy Image
                                untuk paksa kirim sebagai gambar, atau Copy
                                Sticker untuk format sticker jika platform/file
                                mendukung.
                              </p>
                            </div>
                          </div>
          ...
          {tab === 'complaints' && (
            <div className='bg-white rounded-2xl p-8 shadow-xl shadow-slate-200/60 border border-slate-100 animate-slide-up'>
              <h3 className='text-xl font-bold text-slate-800'>
                Complaint Form Configuration
              </h3>
              <p className='text-sm text-slate-500 mt-1 mb-6'>
                Definisikan data/formulir yang wajib ditanyakan AI ketika
                pelanggan mengajukan komplain atau keluhan.
              </p>
              ...
              <p className='text-sm text-slate-500 mt-1 mb-6'>
                Secara otomatis teruskan laporan keluhan ke manajer atau akun
                tertentu.
              </p>
              ...
              <div className='flex items-center gap-3 mb-6'>
                ...
                />
                <label
                  htmlFor='complaint-notify-enable'
              ...
              {complaintNotification.enabled && (
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6 animate-slide-up'>
                  ...
                  <div className='flex flex-col gap-2'>
                    <label className='text-xs font-bold text-slate-500 uppercase tracking-wide'>
                      Destination (No. HP atau Chat ID)
                    </label>
                    <input
                      type='text'
            ...
            <div className='space-y-6 animate-slide-up'>
                ...
                <div className='flex items-center gap-3 mb-2'>
                  ...
                  <div>
                    ...
                    <p className='text-slate-400 text-sm'>
                      Konfigurasi model AI khusus untuk agent ini
                    </p>
                  </div>
                </div>
                <div className='mt-4 flex items-center gap-2 bg-slate-700/50 rounded-xl px-4 py-2.5 text-sm text-slate-300'>
                  ...
                  <span>
                    ...
                    <strong className='text-white'>
                      menimpa konfigurasi global
                    </strong>{' '}
                    hanya untuk agent ini. Kosongkan jika ingin menggunakan
              ...
              <div className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100'>
                ...
                <p className='text-xs text-slate-500 mb-4'>
                  Pilih provider AI yang akan digunakan untuk agent ini.
                </p>
                  ...
                  {[
                    {
                      ...
                      icon: 'fa-robot',
                      desc: 'OpenAI API atau provider yang kompatibel (LocalAI, Ollama, dll)',
                    },
                    {
                      id: 'global',
                      label: 'Gunakan Default Sistem',
                      icon: 'fa-globe',
                      desc: 'Gunakan konfigurasi AI global dari server',
                    },
                  ].map((p) => (
                    <button
                      key={p.id}
                      onClick={() =>
                        setAiSettings((prev) => ({ ...prev, provider: p.id }))
                      }
                    ...
                    >
                      ...
                      <div>
                        <div
                          className={`font-semibold text-sm ${aiSettings.provider === p.id ? 'text-orange-700' : 'text-slate-700'}`}
                        >
                          {p.label}
                        </div>
              ...
              {aiSettings.provider === 'openai' && (
                <div className='bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 space-y-5'>
                  <div>
                    ...
                    <p className='text-xs text-slate-500'>
                      Kompatibel dengan OpenAI API, LocalAI, Ollama (via OpenAI
                      compat), LM Studio, Together AI, Groq, dan lainnya.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className='text-xs text-slate-400 mt-1'>
                      Untuk server lokal, bisa diisi dengan nilai apapun (misal:{' '}
                      <code className='bg-slate-100 px-1 rounded'>local</code>)
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className='text-xs text-slate-400 mt-1'>
                      Nama model yang tersedia di provider Anda
                    </p>
                  </div>
                  ...
                  <div className='bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3'>
                    ...
                    <div className='text-sm text-blue-700'>
                      ...
                      <ul className='mt-1 space-y-0.5 text-blue-600 text-xs'>
                        <li>
                          • <strong>LocalAI / LM Studio:</strong> Base URL{' '}
                          <code className='bg-blue-100 px-1 rounded'>
                            http://localhost:PORT/v1
                          </code>
                        </li>
                        <li>
                          • <strong>Ollama:</strong> Base URL{' '}
                          <code className='bg-blue-100 px-1 rounded'>
                        ...
                        <li>
                          • <strong>Groq:</strong> Base URL{' '}
                          <code className='bg-blue-100 px-1 rounded'>
                            https://api.groq.com/openai/v1
                          </code>
                        </li>
                        <li>
                          • <strong>Together AI:</strong> Base URL{' '}
                          <code className='bg-blue-100 px-1 rounded'>
                ...
                <div className='bg-slate-50 border border-slate-200 rounded-2xl p-6 flex gap-4 items-start'>
                  ...
                  <div>
                    <h4 className='font-bold text-slate-700 mb-1'>
                      Menggunakan Konfigurasi Default Sistem
                    </h4>
                    <p className='text-sm text-slate-500'>
                      Agent ini akan menggunakan model dan konfigurasi AI yang
                      dikonfigurasi di file{' '}
                      <code className='bg-slate-100 px-1 rounded'>.env</code>{' '}
                      server. Pergi ke menu <strong>Settings</strong> untuk
              ...
              <div>
                  ...
                  <div className='flex flex-col mb-1.5 px-1.5'>
                    <div className='flex justify-between items-center w-full'>
                      <div className='flex items-center gap-2'>
                        ...
                        <h3 className='font-bold text-slate-800 text-lg leading-none m-0 p-0'>
                          Preview Live
                        </h3>
                      </div>
                      <button
                    ...
                    <p className='text-xs text-slate-400 mt-[2px] font-medium leading-none m-0 p-0'>
                      Lihat bagaimana {name || agent?.name || 'SelaluTeh AI'}{' '}
                  ...
                  <div className='flex-1 border border-slate-100 rounded-[1.5rem] bg-white flex flex-col overflow-hidden relative min-h-0'>
                    ...
                    <div className='px-4 py-2 border-b border-slate-100 flex items-center gap-2.5 bg-white z-10'>
                      ...
                      <div className='flex flex-col justify-center'>
                        <h3 className='font-bold text-slate-800 text-sm leading-none truncate max-w-[150px]'>
                          {name || agent?.name || 'SelaluTeh AI'}
                        </h3>
                    ...
                    <div className='flex-1 bg-white px-6 pt-4 pb-6 overflow-y-auto space-y-2 scroll-smooth min-h-0'>
                      ...
                      <div className='flex items-center justify-center mt-1 mb-1.5'>
                        ...
                        <span className='text-[10px] text-slate-400 font-medium px-3'>
                          Hari ini
                        </span>
                        <div className='flex-1 h-[1px] bg-slate-100'></div>
                      </div>
                        ...
                        return (
                          <div className='flex flex-col items-start max-w-[85%] animate-slide-up'>
                            <div className='inline-block w-fit max-w-full bg-slate-50 border border-slate-100 text-slate-800 px-4 pt-3 pb-5 rounded-2xl rounded-tl-none text-sm leading-relaxed relative shadow-sm'>
                              ...
                              {previewWelcome.text && (
                                <p
                                  className={`${previewWelcome.attachment || stickerUrl ? 'mt-2' : 'mt-0'} mb-0 whitespace-normal break-words`}
                                >
                                  {previewWelcome.text}
                                </p>
                              )}
                      ...
                      {messages.map((m, idx) => {
                        const normalizedMessage = normalizePreviewMessage(
                          m,
                          database
                        )
                        ...
                        return (
                          <div
                            key={idx}
                            className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-full animate-slide-up`}
                          >
                            <div
                              className={`max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed relative shadow-sm pb-6 ${
                                isUser
                            ...
                            >
                              ...
                              {normalizedMessage.text && (
                                <p
                                  className={`${normalizedMessage.attachment || normalizedMessage.sticker ? 'mt-2' : 'mt-0'} mb-0 break-words ${isUser ? 'pr-12' : ''}`}
                                >
                                  {normalizedMessage.text}
                                </p>
                              )}
                    ...
                    <div className='p-6 pt-2 bg-white pb-6 border-t border-slate-100 shrink-0'>
                      {testAttachment && (
                        <div className='flex items-center gap-2 p-2 bg-slate-50 border border-slate-200 rounded-xl mb-2 animate-slide-up'>
                          ...
                          />
                          <span className='flex-1 text-xs truncate font-medium text-slate-600'>
                            {testAttachment.file?.name}
                          </span>
                          <button
                      ...
                      <div className='relative group'>
                        ...
                        <div className='absolute right-2 top-2 bottom-2 flex gap-1'>
                          ...
                          <button
                            onClick={() =>
                              document
                                .getElementById('test-photo-input')
                                ?.click()
                            }
                ...
                <div className='text-center mt-3 shrink-0'>
                  <p className='text-[10px] text-slate-400 font-medium'>
                    Preview ini menggunakan contoh respons AI. Hasil aktual
                    dapat berbeda.
                  </p>
                </div>
              </div>
