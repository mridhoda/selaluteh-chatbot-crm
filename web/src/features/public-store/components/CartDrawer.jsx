        ...
        <div className='flex-1 overflow-y-auto pb-24'>
          {!hasItems ? (
            <div className='flex flex-col items-center justify-center py-16 px-6 text-center'>
              ...
              <button
                type='button'
                className='mt-6 rounded-full bg-[var(--brand-500)] px-6 py-2.5 text-sm font-bold text-white shadow-md active:scale-95 transition-transform'
                onClick={onClose}
              >
                Mulai Pilih Menu
              </button>
              ...
              <div className='p-4 space-y-3'>
                ...
                <div className='flex justify-between text-sm text-gray-600'>
                  ...
                  <span className='font-medium text-gray-900'>
                    {formatCurrency(cart.totals.subtotalMinor)}
                  </span>
                </div>
                {cart.totals.serviceFeeMinor > 0 && (
                  <div className='flex justify-between text-sm text-gray-600'>
                    ...
                    <span className='font-medium text-gray-900'>
                      {formatCurrency(cart.totals.serviceFeeMinor)}
                    </span>
                  </div>
                )}
                {cart.totals.discountMinor > 0 && (
                  <div className='flex justify-between text-sm text-emerald-600'>
                    ...
                    <span className='font-medium'>
                      - {formatCurrency(cart.totals.discountMinor)}
                    </span>
                  </div>
                )}
                <div className='pt-3 border-t border-gray-100 flex justify-between font-bold text-base text-gray-900'>
                  ...
                  <span className='text-[var(--brand-600)]'>
                    {formatCurrency(cart.totals.totalMinor)}
                  </span>
                </div>
              </div>
            </>
