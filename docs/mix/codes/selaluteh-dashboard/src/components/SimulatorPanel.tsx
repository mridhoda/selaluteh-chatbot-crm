...
import { Order, OrderStatus, ChannelType } from '../types';
import { MOCK_CHATS, MockChatMessage } from '../data';

interface SimulatorPanelProps {
  onSimulateOrder: (order: Omit<Order, 'id' | 'orderNo' | 'timestamp'>) => void;
  activeOrders: Order[];
  onUpdateOrderStatus: (orderId: string, status: OrderStatus) => void;
  onResolveChat: (chatId: string, isAi: boolean) => void;
  chats: MockChatMessage[];
...
const CHANNELS: ChannelType[] = ['WhatsApp', 'Telegram', 'Instagram', 'Website'];
...
export default function SimulatorPanel({
  ...
  const [activeTab, setActiveTab] = useState<'order' | 'kitchen' | 'chat'>('order');

  // Order Simulation State
  const [customerName, setCustomerName] = useState('');
  const [selectedOutlet, setSelectedOutlet] = useState(OUTLETS[0]);
  const [selectedChannel, setSelectedChannel] = useState(CHANNELS[0]);
  const [selectedItems, setSelectedItems] = useState<Array<{ name: string; qty: number; price: number }>>([]);
  ...
  const handleCreateOrderSubmit = (status: OrderStatus = 'Incoming') => {
