...
export default function StatusChart({
  ...
  const [hoveredSegment, setHoveredSegment] = useState<OrderStatus | null>(null);
  const [timeframe, setTimeframe] = useState('Hari Ini');

  const total = incoming + preparing + ready + completed;

  const data: Array<{ label: OrderStatus; count: number; color: string; bgClass: string }> = [
  ...
  return (
    <motion.div
      ...
      className="card p-6 bg-white border border-slate-100 rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex flex-col justify-between h-full"
    >
      {/* Header */}
