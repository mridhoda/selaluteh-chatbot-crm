...
const { cartsSupabaseRepository: cartsRepository } = await import('../../src/db/repositories/carts.supabase.repository.js');

const testUuid = () => crypto.randomUUID();

describe('checkout flow (Supabase integration)', () => {
  let client, wsId, outletId, productId, contactId, chatId, platformId;
  ...
  before(async () => {
    ...
    // Seed: platform, contact, and chat
    await client.from('platforms').insert({
      id: platformId, workspace_id: wsId, type: 'telegram', label: 'Telegram Bot', status: 'connected'
    });
    await client.from('contacts').insert({
      id: contactId, workspace_id: wsId, name: 'Test User', external_id: 'test_tg_123', platform_id: platformId
    });
    await client.from('chats').insert({
      id: chatId, workspace_id: wsId, contact_id: contactId,
      platform_id: platformId, status: 'open',
  ...
  after(async () => {
    ...
    await client.from('platforms').delete().eq('workspace_id', wsId);
    await client.from('product_outlet_availability').delete().eq('workspace_id', wsId);
    await client.from('outlet_operating_hours').delete().eq('workspace_id', wsId);
  ...
  it('Step 2: Cart is created with item', async () => {
    ...
    await client.from('cart_items').insert({
      ...
      product_name_snapshot: 'Teh Tarik Original',
      quantity: 1, unit_price: 15000, subtotal_amount: 15000
    });
